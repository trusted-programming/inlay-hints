            ...
            for (expr_id, _) in body.exprs.iter() {
                ...
                let unknown_or_partial = if ty.is_unknown() {
                    ...
                    if verbosity.is_spammy() {
                            ...
                            expr_syntax_range(db, &analysis, vfs, &sm, expr_id)
                        {
                            println!(
                                "{:?}: {:?} @ {} {}:{}-{}:{}",
                                expr_id, ty, path, start.line, start.col, end.line, end.col
                            );
                            bar.println(format!(
                                "{} {}:{}-{}:{}: Unknown type",
                                path,
                    ...
                    if verbosity.is_spammy() {
                        if let Some((path, start, end)) =
                            expr_text(db, &analysis, vfs, &sm, expr_id)
                        // expr_syntax_range(db, &analysis, vfs, &sm, expr_id)
                        {
                            println!(
                                "{:?}: {:?} @ {} {}:{}-{}:{}",
                ...
                if self.only.is_some() && verbosity.is_spammy() {
                        ...
                        expr_syntax_range(db, &analysis, vfs, &sm, expr_id)
                    {
                        println!(
                            "{:?}: {:?} @ {}:{}-{}:{}",
                            expr_id, ty, start.line, start.col, end.line, end.col
                        );
                        bar.println(format!(
                            "{}:{}-{}:{}: {}",
                            start.line + 1,
...
fn expr_text(
    ...
    if let Ok(src) = src {
        ...
        let node = src.map(|e| e.to_node(&root).syntax().clone());
        // FIXME
        println!("{:?}", node.as_ref().value.text());
        let original_range = node.as_ref().original_file_range(db);
        let path = vfs.file_path(original_range.file_id);
        // FIXME
        // analysis.inlay_hints(&config, file_id, original_range)?;
        let line_index = analysis.file_line_index(original_range.file_id).unwrap();
        let text_range = original_range.range;
        let (start, end) =
            (line_index.line_col(text_range.start()), line_index.line_col(text_range.end()));
